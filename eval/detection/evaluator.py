...
class Evaluator:
    def GetPascalVOCMetrics(
        ...
        for c in classes:
            ...
            dects = []
            [dects.append(d) for d in detections if d.getClassId() == c]
            ...
            for g in groundTruths:
                if g.getClassId() == c:
                    npos += 1
                    gts[g.getImageName()] = gts.get(g.getImageName(), []) + [g]
                        
            # sort detections by decreasing confidence
            dects = sorted(dects, key=lambda d: d.getConfidence(), reverse=True)
            ...
            for d in range(len(dects)):
                score[d] = dects[d].getConfidence()

                # Find ground truth image
                gt = gts[dects[d].getImageName()] if dects[d].getImageName() in gts else []
                iouMax = sys.float_info.min
                
                for j in range(len(gt)):
                    iou = Evaluator.iou(dects[d].getAbsoluteBoundingBox(BBFormat.XYX2Y2), 
                                        gt[j].getAbsoluteBoundingBox(BBFormat.XYX2Y2))
                    if iou > iouMax:
                        iouMax = iou
                        jmax = j
                # Assign detection as true positive/don't care/false positive
                if iouMax >= IOUThreshold:
                    if det[dects[d].getImageName()][jmax] == 0:
                        TP[d] = 1  # count as true positive
                        det[dects[d].getImageName()][jmax] = 1  # flag as already 'seen'
                    else:
